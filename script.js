document.addEventListener('DOMContentLoaded', () => {
    ...
    const staffNameTranslations = {
        'NAKAYAMA': '中山', 'TAKAMI': '髙見', 'KANEKO A.': '金子 愛', 'KANEKO T.': '金子 智',
        'KIYONAGA': '清永', 'IWAI': '岩井', 'MURABAYASHI': '村林', 'ZHANG': '張',
        'YEN': 'グエン', 'BIJAYA': 'ビザヤ', 'MARK NOLAN': 'マークノラン', 'UTSUGI': '宇津木',
        'MIYAKAWA': '宮川', 'KOKUBU': '国分', 'SAKAMOTO': '坂本', 'OGAWA': '小川',
        'GUSHIKEN': '具志堅'
    };
    ...
    const letterContent = {
        en: { title: 'Room Cleaning Notification', body: `<p>Thank you for staying with us.</p><p>In accordance with hotel policy, we clean rooms that have not been serviced for two consecutive days. Therefore, our staff will be entering your room for scheduled cleaning tomorrow between 10:00 AM and 2:00 PM.</p><p>Please place the “Clean Up” tag on your door. If you have any questions, please feel free to contact us.</p><p>We appreciate your understanding and cooperation.</p>` },
        ja: { title: '客室清掃のお知らせ', body: `<p>ご宿泊いただき、誠にありがとうございます。</p><p>当ホテルのポリシーに基づき、2日間清掃が行われなかったお部屋は、翌日に清掃を実施しております。つきましては、明日の午前10時から午後2時の間に、スタッフがお部屋の清掃に入室いたします。</p><p>お部屋のドアに「清掃してください」の札をお出しください。ご不明な点がございましたら、お気軽にお問い合わせください。</p><p>何卒、ご理解ご協力のほど、よろしくお願い申し上げます。</p>` }
    };

    const letterLabels = {
        en: { roomNo: 'Room No:', guestName: 'Guest Name:', clerk: 'Clerk', date: 'Date:', frontDeskCopy: '[ FRONT DESK COPY ]' },
        ja: { roomNo: '部屋番号:', guestName: 'お客様名:', clerk: '担当者', date: '日付:', frontDeskCopy: '[ フロント控え ]' }
    };
    ...
    const initialStepsContainer = document.getElementById('initial-steps-container'); // Container for steps 1 & 2
    const showUploadBtn = document.getElementById('show-upload-btn'); // Floating button
    ...
    const parseCSV = (csvText) => {
        ...
        lines.forEach((line, index) => {
            ...
            if (roomColumn && nameColumn && !containsNumber(nameColumn)) {
                if (!data[roomColumn]) data[roomColumn] = [];
    ...
    // UPDATED: Logic to hide initial steps and show floating button
    const handleFileUpload = (event) => {
        ...
        reader.onload = (e) => {
            ...
            if (recordCount > 0) {
                ...
                letterGenerationSection.classList.remove('hidden'); // Show steps 3 & 4
                initialStepsContainer.classList.add('hidden'); // Hide steps 1 & 2
                showUploadBtn.classList.remove('hidden'); // Show floating button
                ...
                letterGenerationSection.classList.add('hidden'); // Hide steps 3 & 4
                initialStepsContainer.classList.remove('hidden'); // Show steps 1 & 2
                showUploadBtn.classList.add('hidden'); // Hide floating button
            }
        };
        reader.onerror = () => {
             uploadStatus.textContent = '❌ Error: Could not read the file.';
             uploadStatus.style.color = '#dc3545';
             initialStepsContainer.classList.remove('hidden');
    ...
    const addRoomRow = () => {
        ...
        roomRow.innerHTML = `<label for="room-${rowId}">部屋番号:</label><input type="text" id="room-${rowId}" class="room-number-input" placeholder="例：501"><span class="guest-name-display">[お客様名]</span><button type="button" class="delete-room-btn" aria-label="Delete room">X</button>`;
    ...
    const handleRoomInput = (event) => {
        ...
        if (names && names.length > 0) {
            ...
            nameDisplay.style.color = '#495057'; nameDisplay.style.fontStyle = 'normal';
        } else {
            nameDisplay.textContent = 'お客様が見つかりません';
            nameDisplay.style.color = '#dc3545'; nameDisplay.style.fontStyle = 'italic';
        }
    };
    ...
    const generateLetters = () => {
        ...
        if (!clerkName) {
            alert('担当者名を入力してください。'); clerkNameInput.focus(); return;
        }
        printArea.innerHTML = '';
        ...
        roomRows.forEach(row => {
            ...
            if (guestNames && guestNames.length > 0) {
                ...
                let formattedGuestName = (language === 'ja') ? combinedGuestName + ' 様' : 'Mr./Ms. ' + combinedGuestName;
                let finalClerkName = clerkName;
                if (language === 'ja') {
                    const translatedName = staffNameTranslations[clerkName.toUpperCase()];
                    if (translatedName) finalClerkName = translatedName;
                } else if (language === 'en') {
                    const translatedName = japaneseToEnglish[clerkName];
                    if (translatedName) finalClerkName = translatedName;
                    else finalClerkName = clerkName.toUpperCase();
                }
                const content = letterContent[language]; const labels = letterLabels[language];
    ...
    // NEW: Event listener for the floating button
    showUploadBtn.addEventListener('click', () => {
        initialStepsContainer.classList.remove('hidden'); // Show steps 1 & 2
        showUploadBtn.classList.add('hidden'); // Hide the floating button
        uploadStatus.textContent = 'エクスポートしたCSVファイルをアップロードしてください。'; // Reset status message
        uploadStatus.style.color = '#6c757d'; // Reset status color
    });
